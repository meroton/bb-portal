...
func StartGrpcWebProxyServer(
	...
	registerAndStartServer(
		...
		func(grpcServer *go_grpc.Server, grpcClient go_grpc.ClientConnInterface) {
			listPlatformQueueAuthorizer, err := auth.DefaultAuthorizerFactory.NewAuthorizerFromConfiguration(configuration.ListPlatformQueueAuthorizer)
			if err != nil {
				log.Fatalf("Failed to create ListPlatformQueueAuthorizer: %v", err)
			}
			c := buildqueuestate.NewBuildQueueStateClient(grpcClient)
			buildqueuestate.RegisterBuildQueueStateServer(grpcServer, buildqueuestateproxy.NewBuildQueueStateServerImpl(c, listPlatformQueueAuthorizer))
		},
	)
	...
	registerAndStartServer(
		...
		func(grpcServer *go_grpc.Server, grpcClient go_grpc.ClientConnInterface) {
			...
			remoteexecution.RegisterActionCacheServer(grpcServer, actioncacheproxy.NewAcctionCacheServerImpl(c))
		},
	)
	...
	registerAndStartServer(
		...
		func(grpcServer *go_grpc.Server, grpcClient go_grpc.ClientConnInterface) {
			...
			bytestream.RegisterByteStreamServer(grpcServer, casproxy.NewCasServerImpl(c))
		},
	)
	...
	registerAndStartServer(
		...
		func(grpcServer *go_grpc.Server, grpcClient go_grpc.ClientConnInterface) {
			...
			iscc.RegisterInitialSizeClassCacheServer(grpcServer, isccproxy.NewIsccServerImpl(c))
		},
	)
	...
	registerAndStartServer(
		...
		func(grpcServer *go_grpc.Server, grpcClient go_grpc.ClientConnInterface) {
			...
			fsac.RegisterFileSystemAccessCacheServer(grpcServer, fsacproxy.NewFsacServerImpl(c))
		},
	)
}
