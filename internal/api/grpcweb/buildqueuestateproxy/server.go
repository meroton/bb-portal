...
func (s *BuildQueueStateServerImpl) GetOperation(ctx context.Context, req *buildqueuestate.GetOperationRequest) (*buildqueuestate.GetOperationResponse, error) {
	...
	if !grpcweb.IsInstanceNamePrefixAllowed(ctx, s.authorizer, response.Operation.InvocationName.SizeClassQueueName.PlatformQueueName.InstanceNamePrefix) {
		return nil, status.Errorf(codes.NotFound, "Operation was not found")
	}
...
func filterPlatormQueues(ctx context.Context, response *buildqueuestate.ListPlatformQueuesResponse, authorizer auth.Authorizer) []*buildqueuestate.PlatformQueueState {
	...
	for _, queue := range queues {
		if grpcweb.IsInstanceNamePrefixAllowed(ctx, authorizer, queue.GetName().GetInstanceNamePrefix()) {
...
func filterOperations(ctx context.Context, response *buildqueuestate.ListOperationsResponse, authorizer auth.Authorizer) []*buildqueuestate.OperationState {
	...
	for _, operation := range operations {
		if grpcweb.IsInstanceNamePrefixAllowed(ctx, authorizer, operation.InvocationName.SizeClassQueueName.PlatformQueueName.InstanceNamePrefix) {
...
func getInstanceNamePrefixFromListWorkersRequest(req *buildqueuestate.ListWorkersRequest) (string, error) {
	...
	switch {
	case filter.GetAll() != nil:
		instanceNamePrefix = filter.GetAll().PlatformQueueName.InstanceNamePrefix
	case filter.GetExecuting() != nil:
		instanceNamePrefix = filter.GetExecuting().SizeClassQueueName.PlatformQueueName.InstanceNamePrefix
	case filter.GetIdleSynchronizing() != nil:
		instanceNamePrefix = filter.GetIdleSynchronizing().SizeClassQueueName.PlatformQueueName.InstanceNamePrefix
	default:
		return "", status.Errorf(codes.InvalidArgument, "Request does not contain a valid InstanceNamePrefix")
	}

	return instanceNamePrefix, nil
}
